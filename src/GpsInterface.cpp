      ...
      if (system)
      {
        if (msg_id == "TXT")
        {
          if (length > 8)
          {
            ...
            if (tot_brk != std::string::npos && num_brk != std::string::npos && txt_brk != std::string::npos && chk_brk != std::string::npos && chk_brk > txt_brk && txt_brk > num_brk && num_brk > tot_brk && tot_brk >= 0)
            {
...
String GpsInterface::generateType()
{
  ...
  { // type_flag=0
    ...
    msg_type += 'B';
  else if (this->type_flag == GPSTYPE_BEIDOU_BD)
  { // type_flag=8
    msg_type += 'B';
    msg_type += 'D';
  }
  else
  {                           // type_flag=1=all ... also default if unset/wrong (obj default is type_flag=0=native)

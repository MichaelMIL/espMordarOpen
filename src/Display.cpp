...
void Display::tftDrawGraphObjects(byte x_scale)
{
  ...
  tft.setCursor(3, 228);
  tft.print("0"); // "-" at bottom of y axis
}
...
void Display::tftDrawEapolColorKey()
{
  ...
  tft.setTextSize(1);
  tft.setTextColor(TFT_WHITE);
  tft.fillRect(14, 0, 15, 8, TFT_CYAN);
  tft.setCursor(30, 0);
  tft.print(" - EAPOL");
}
...
void Display::tftDrawColorKey()
{
  ...
  tft.setTextSize(1);
  tft.setTextColor(TFT_WHITE);
  tft.fillRect(14, 0, 15, 8, TFT_GREEN);
  tft.setCursor(30, 0);
  tft.print(" - Beacons");
  tft.fillRect(14, 8, 15, 8, TFT_RED);
  tft.setCursor(30, 8);
  tft.print(" - Deauths");
  tft.fillRect(14, 16, 15, 8, TFT_BLUE);
  tft.setCursor(30, 16);
  tft.print(" - Probes");
}
...
void Display::tftDrawChannelScaleButtons(int set_channel)
{
  ...
  tft.setCursor(145, 21);
  tft.setTextColor(TFT_WHITE);
  tft.setTextSize(1);
  tft.print(text10);
  tft.print(set_channel);
...
int Display::scroll_line(uint32_t color)
{
  ...
  else
  {
    ...
    // The value must wrap around as the screen memory is a circular buffer
    if (yStart >= YMAX - BOT_FIXED_AREA)
      yStart = TOP_FIXED_AREA_2 + (yStart - YMAX + BOT_FIXED_AREA);
  }
